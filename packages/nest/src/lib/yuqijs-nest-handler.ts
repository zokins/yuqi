...
export class ResponseValidationError extends InternalServerErrorException {
  constructor(
    ...
    super(
      `[Yuqi] Response validation failed for ${appRoute.method} ${appRoute.path}: ${error.message}`,
    );
  }
}
...
const validateResponse = (
  ...
  if (!responseSchema) {
    throw new InternalServerErrorException(
      `[Yuqi] Couldn't find a response schema for ${response.status} on route ${appRoute.path}`,
    );
  }
