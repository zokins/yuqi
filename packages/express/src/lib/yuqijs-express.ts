  ...
  if (typeof router === "object" && typeof router?.handler !== "function") {
    for (const key in router) {
      if (isAppRoute(schema)) {
        throw new Error(`[Yuqi] Expected AppRouter but received AppRoute`);
      }
    ...
    if (!isAppRoute(schema)) {
      throw new Error(`[Yuqi] Expected AppRoute but received AppRouter`);
    }
...
const initializeExpressRoute = ({
  ...
  if (options.logInitialization) {
    console.log(`[Yuqi] Initialized ${schema.method} ${schema.path}`);
  }
